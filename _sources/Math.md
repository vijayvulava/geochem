...
In this appendix, Let's review basic algebra and calculus, which are often useful in environmental geosciences.
...
In geosciences, you will work with extremely large to minuscule numbers. The digits required to describe those numbers increase as the numbers get bigger or smaller. For example, the number of people on Earth is approximately $\pu{8 billion}$, or $\pu{8 000 000 000}$. Nanoparticles refer to particles with a size of $\pu{0.000 000 0001 m}$, or a billionth of a meter. Manipulating large and small numbers is unwieldy, and errors creep in if we forget a digit (i.e., a factor of $10$).

To overcome this problem, scientists have developed a shorthand notation for numbers using exponents. The general form is

$$C \times B^E \quad \text{or} \quad C \cdot B^E $$

where $C$ is the coefficient, $B$ is the base number and $E$ is the exponent. "$\times$" and "$\cdot$" can signify the multiplication of the $C$ and $B$. This textbook uses "$\cdot$" for scientific notation.
...
List commonly used prefixes in the **International System of Units** (SI units).
...
The number of digits shown for a measurement implies the level of measurement certainty. The number of significant figures is the number of digits provided. As the number of significant figures increases, the measurement becomes more certain.
...
Two rules apply to calculations involving significant figures:
1. In multiplication and division, round the final result to the least significant figures of any one term.
...
For example, $\pi = \pu{3.141 592 65}$ is quoted to 9 significant figures and 8 decimal places. If we quote $\pi = \pu{3.14}$ (3 sig. fig.), this is smaller than the more extended number, so we have rounded down. If we choose $\pi = \pu{3.142}$ (4 sig. fig.), this is larger than the more extended number, so we have rounded up. Generally, if the digit beyond the required significant figure is larger than or equal to 5, we round up; if it is less than 5, we round down.
```


### Units and Dimensions

Everything we measure has a unit associated with it: meters, degrees Celsius (or kelvin), seconds, etc. The best practice is to state units for all answers. The dimension of a physical quantity indicates explicitly how the quantity is related, through its defining equation, to the fundamental quantities.

Fundamental quantities of interest in this book include length $[L]$, which is measured in meters; time $[T]$, which is measured in seconds; and mass $[M]$, which is measured in kilograms. Whenever a calculation is performed, the dimension of the equation must remain consistent.
...
In geosciences, the subscript is typically used to help clarify the meaning of a variable. For example, a rock's density ($\rho$, rho) can be written as $\rho_\text{rock}$.
...
When numbers are encountered in a format such as $X^A$, it denotes some number $X$ to the *power of $A$*, where $A$ can be a whole or fractional number (e.g., 2 or 1/3). $A$ is the exponent (or power or index), and $X$ is the base. Here are some examples:
...
In the expressions below, a common factor was identified and isolated.
...
‘Transpose’ means rearrange. The general rule is: *whatever you do to the equation's left-hand side (LHS), you do to the right-hand side (RHS) so they remain equal*.

For example, the area $A$ of a circle is related to its radius $r$ as $A = \pi r^2$. Here, $A$ is the subject of the equation. If we want to know the radius corresponding to a circle area of $\pu{10 m2}$, we must rearrange the equation so that $r$ is the subject. 
...
When you take the square root of a number, you have two possible answers (positive and negative) because, in reverse, when you square the negative number, you get the same answer as squaring the positive number. Typically, it is clear which root to use in subsequent calculations, but you should generally report the positive and negative roots.

## Solving equations

### Linear equations

A linear equation is one of the forms $ax + b = y$, where $a$ and $b$ are numbers, and the unknown quantity is $x$. Some definitions:
...
The horizontal number line is called the ***x-axis***. The vertical number line is called the ***y-axis***. The _x_-axis and the _y_-axis together form the rectangular coordinate system. These axes divide a plane into four regions, called **quadrants**. We only need two points on a line for linear equations to define the equation.
...
Except for special cases where $b = 0$ or $c = 0$, factoring by inspection only works for quadratic equations with rational roots. This means that most quadratic equations that arise in practical applications cannot be solved by factoring and inspection.
...
We illustrate the use of this algorithm by solving $2x^2 + 4x - 4 = 0$
...
> Note that both roots have to be evaluated in the above solution with the problem being considered. 
...
If the parabola intersects the $x$-axis in two points, there are two real roots, the $x$-coordinates of these two points (also called $x$-intercept).
...
If the parabola is tangent to the $x$-axis, there is a double root, the $x$-coordinate of the contact point between the graph and the parabola.
...
1. Simplify equations by collecting the common terms.
2. Eliminate one of the unknowns by (a) subtracting (adding) one equation from (to) the other; (b) or rearranging the equations so that each has the same subject (e.g., x = ?).
...
2. $\sum$ is shorthand for ‘summation,’ so we are summing successive values of $x$ in this equation.
3. The numbers on the bottom and top of $\sum$ are called the limits of the sum. In our present example, we sum from $i = 1$ to $i = n$. For our numerical example that sums the first 10 numbers (excluding zero), $n = 10$.
4. The bar above $\bar{x}$ denotes the mean value of $x$.

An alternative method of describing data is using the ***median*** value, which is the number separating the higher half of a dataset from the lower half. This is obtained by ranking the data in order of magnitude and taking the central value; take the mean of the middle two numbers if you have an even number of points. A significant difference between the mean and median of a dataset quickly highlights this problem.

The ***variance*** of a dataset measures the spread of possible values, averaging the squared distance of individual values from the mean value. This is opposed to the mean, which is a way to describe the location of a distribution. The following formula, using the notation outlined above, describes the sample variance, denoted by $\sigma^2$:

$$\sigma^2 = \frac{1}{n-1} \sum_{i=1} ^n (\bar{x}-x_i)^2
$$

In other words, this is the mean square deviation from the mean. The ***standard deviation*** (often denoted by $\sigma$) of a dataset is found by taking the square root of the variance:
...
Exponential notation expresses very large and very small numbers as a product of two numbers. The first number of the product, the _digit term_, is usually a number not less than 1 and not greater than 10. The second number of the product, the _exponential term_, is written as 10 with an exponent. 

An expression of the form $a^x$ is called an exponential expression. The number $a$ is the base, and $x$ is the power, exponent, or index. Exponential growth occurs when the growth becomes more rapid in proportion to the increase in the quantity; exponential decay can be treated as a negative growth rate.
...
To add $\pu{5.00e-5}$ with $\pu{3.00e-3}$, convert one of the numbers to the same exponential as the other:
...
To subtract $\pu{4.0e-7}$ from $\pu{5.0e-6}$, convert one of the numbers to the same exponential as the other:
...
```{admonition} Example of the square root of an exponential number
Find the square root of $\pu{1.6e-7}$.
...
Logarithms provide a way to succinctly describe data with a wide range of values. Within geosciences, we work from the nanoscale ($\pu{e-9 m}$) to the planetary scale ($\pu{e6 m}$), straddling more than 15 orders of magnitude. Essentially, logarithms provide a shortcut for exponents.
...
The most common bases are base $10$ and the base $e$. The mathematical constant $e$ has a $\pu{2.71828}$ value and occurs in many natural phenomena, such as radioactive decay. We will follow the convention that logs to the base 10 are denoted by $\log$ or $\log_{10}$ and logs to the base $e$ (referred to as natural logarithms) are denoted by $\ln$ or $\log_e$. Get familiar with the $\log$ and $\ln$ buttons on your calculator.

```{admonition} Three main logarithm laws
There are three main logarithm laws (apply to both $\log$ and $\ln$):
...
Calculus is the study of how things change and provides a framework for modeling phenomena (natural or otherwise) in which there is change and a way to deduce the predictions of such models. Calculus is essential for geologists because it helps them understand the behavior of natural systems, such as the movement of groundwater through aquifers, the deformation of rocks under stress, and the flow of heat through the Earth’s interior. Calculus is also used to model the behavior of natural systems, which can help geologists make predictions about future events.

Calculus provides an approach to construct relatively simple quantitative change models and deduce their consequences. The Earth system is ever-changing on a broad spectrum of temporal and spatial scales. Without calculus, we are limiting our understanding of the natural world. We can better understand the effects of changing conditions on the phenomena under investigation using calculus.

In calculus, we are dealing with incremental changes in quantities. These changes are typically denoted by $\Delta$ and $d$ so that $\Delta x = dx$ is a small change in the variable $x$.

Differential calculus is the branch of calculus that deals with the study of rates of change and slopes of curves. It is used to find a function's instantaneous rate of change at a given point. A function's derivative is the tangent line's slope to the function at a given point.

Integral calculus is the branch of calculus that deals with the study of areas under and between curves. It is used to find a function's total change or accumulation over an interval. The definite integral of a function is the area under the curve of the function between two points.

### Differential Calculus

The derivative of a function and its applications are usually the main concepts. The derivative of a function at a chosen input value describes *the rate of change of the function near that input value*. The process of finding a derivative is called *differentiation*. Geometrically, the derivative at a point is the tangent line's slope to the function's graph at that point, provided that the derivative exists and is defined at that point. For a real-valued function of a single real variable, the derivative of a function at a point generally determines the best linear approximation to the function at that point.

#### Derivative

A derivative measures how much a function changes as its input changes. This is the derivative of a function $f$ at a point $x$ is the slope of the line tangent to the graph of $f$ at $x$. The derivative of the function $f(x)$ is denoted by $f'(x)$ or $dy/dx$ and is defined as the limit of the ratio of the change in y to the change in $x$ as the change in $x$ approaches zero. Mathematically, this can be expressed as:
