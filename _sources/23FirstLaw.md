...
In Eq {eq}`PV`  $P$ is the external, opposing pressure, and $\Delta V$ is the change in the container volume due to the piston being pushed upward. The negative sign in Eq {eq}`PV`  accounts for an increase in volume, resulting in a negative value for $w$. At constant $V$, ($\Delta V = 0$), $q_V = _{sys}{E}$. 
...
Enthalpy is a state function derived from rearranging Eq {eq}`EnergyWork` as follows:
...
````{margin}
The CRC Handbook of Physics and Chemistry is a good reference for obtaining $\Delta_f H^\circ$ for most chemicals. The online version is available at [Standard Thermodynamic Properties of Chemical Substances (chemnetbase.com)](https://hbcp.chemnetbase.com/faces/documents/05_02/05_02_0001.xhtml)
````
